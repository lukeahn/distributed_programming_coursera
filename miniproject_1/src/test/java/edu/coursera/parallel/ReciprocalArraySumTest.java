...
public class ReciprocalArraySumTest extends TestCase {
    ...
    public void testParSimpleTwoMillion() {
        ...
        final String errMsg = String.format("T1. It was expected that the two-task parallel implementation would run at " +
    ...
    public void testParSimpleTwoHundredMillion() {
        ...
        final double minimalExpectedSpeedup = 1.5;
        final String errMsg = String.format("T2. It was expected that the two-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    }
    ...
    public void testParManyTaskTwoMillion() {
        ...
        final String errMsg = String.format("T3. It was expected that the many-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    }
    ...
    public void testParManyTaskTwoHundredMillion() {
        ...
        final String errMsg = String.format("T4. It was expected that the many-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    }
}
